...
export default function EstimatePanel({ project, systems, onOpenConfirm, onReset }: EstimatePanelProps) {
    ...
    if (s.sysType === 'mini') {
      rows.push(
        <div className="est-sys-line" key="base">
          <span>{miniBtuLabel(s)} system · <span style={{ color: '#7d8e8b' }}>{miniHeadTypeName(s)}</span></span>
      ...
      rows.push(
        <div className="est-sys-line" key="base">
          <span>Multi-Split system · <span style={{ color: '#7d8e8b' }}>{multiCondenserBracketLabel(s)} · {headsCount} head{headsCount !== 1 ? 's' : ''}</span></span>
      ...
      if (s.zoned) {
        ...
        rows.push(
          <div className="est-sys-line" key="zone">
            <span>Multi-Zone Damper System · <span style={{ color: '#7d8e8b' }}>{z} zones · $3,100{addlTxt}{nestTxt}</span></span>
  ...
  const buildPrintDoc = () => {
    ...
    const logoSrc = (document.querySelector('.logo-img') as HTMLImageElement)?.src || '';

    const printSysHtml = systems.map((s, i) => {
      const brand = getBrand(s);
      const sub = systemSubtotal(s, project, systems.length);
      const base = baseForSystem(s, project);
      const nestOk = brand.nest;
      ...
      if (s.sysType === 'mini') {
        lines.push(`<div class="pd-line"><span>${esc(`${brand.name} ${miniBtuLabel(s)} mini-split system (${miniHeadTypeName(s)})`)}</span><span class="pl-amt">${fmt(base)}</span></div>`);
      } else if (s.sysType === 'multi') {
        const headsCount = (s.heads || []).length;
        const hTotal = headsTotal(s, project.tier);
        ...
        if (eff && eff.delta > 0) {
          lines.push(`<div class="pd-line"><span>${esc(`${eff.name} efficiency upgrade (${eff.desc})`)}</span><span class="pl-amt">${fmt(eff.delta)}</span></div>`);
        }
        if (!s.zoned && nestOk && s.singleNest) {
          lines.push(`<div class="pd-line"><span>Google Nest (4th Gen) or Equal</span><span class="pl-amt">${fmt(NEST_RATE)}</span></div>`);
        }
        if (s.zoned) {
          const z = Math.max(s.zoneCount || 2, 2);
          const showNest = nestOk && s.multiNest;
          const zc = ZONE_FIRST + Math.max(z - 1, 0) * ZONE_ADDL + (showNest ? NEST_RATE * z : 0);
          lines.push(`<div class="pd-line"><span>Multi-Zone Damper System (${z} zones)${showNest ? ' + Nest' : ''}</span><span class="pl-amt">${fmt(zc)}</span></div>`);
        }
      }
      ...
      SYSTEM_ADDON_DEFS.forEach(def => {
        ...
        const lt = addonLineTotal(def, a, systems.length, s);
        const sl = addonSubline(def, a, s);
        lines.push(`<div class="pd-line"><span>${esc(def.name + (sl ? ` (${sl})` : ''))}</span><span class="pl-amt">${fmt(lt)}</span></div>`);
      });
      ...
      if (isDuctless(s)) {
        ...
        const warrantyFull = eff?.warranty + (eff?.unitReplacement ? ' · ' + eff.unitReplacement : '');
        spec = `<div class="pd-spec">${esc(eff?.series || '')} · ${esc(eff?.seer2 || '')} · ${esc(warrantyFull)}</div>`;
      }

      const typeDesc = s.sysType === 'mini' ? 'Mini-Split' : s.sysType === 'multi' ? `Multi-Split · ${(s.heads || []).length} heads` : `${s.tons} ton`;
      const note = s.notes?.trim() ? `<div class="pd-note">Note: ${esc(s.notes.trim())}</div>` : '';
    ...
    PROJECT_ADDON_DEFS.forEach(def => {
      ...
      if (a && a.on) {
        ...
        if (lt > 0 || def.type !== 'multirow') {
          ...
          projAddonsLines.push(`<div class="pd-line"><span>${esc(def.name + (sl ? ` (${sl})` : ''))}</span><span class="pl-amt">${fmt(lt)}</span></div>`);
          printProjTotal += lt;
        }
      }
    });

    const printProjHtml = projAddonsLines.length > 0
    ...
    const grandTotal = systems.reduce((acc, s) => acc + systemSubtotal(s, project, systems.length), 0) + printProjTotal;
    const revised = project.revisedFrom ? `<div class="m"><span class="ml">Revised from</span><span class="mv q">${esc(project.revisedFrom)}</span></div>` : '';
  ...
  return (
    <div className="estimate">
      ...
      <div className="est-actions">
        <button type="button" className="btn btn-primary" id="panelPrintBtn" onClick={handlePrint}>Print / Save PDF</button>
        <button type="button" className="btn btn-ghost" id="resetBtn" onClick={onReset}>Reset</button>
      </div>
