...
export default function ProjectAddons({ project, onChange }: ProjectAddonsProps) {
  ...
  const selCount = PROJECT_ADDON_DEFS.filter(d => project.addons[d.id]?.on).length;
  ...
  return (
    <>
      ...
      <div className={`proj-group ${open ? 'open' : ''}`} id="projGroup">
        <button type="button" className="proj-head" id="projHead" onClick={() => setOpen(!open)}>
          <div className="proj-head-info">
            <div className="proj-title">More project add-ons</div>
            <div className="proj-sub" id="projHeadSub">{selCount ? `${selCount} selected` : 'Ductwork, crawl space, riser & flex duct'}</div>
          </div>
