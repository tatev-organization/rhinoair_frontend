...
export default function AdminProjectDetailsPage({
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetProjectByIdQuery(unwrappedParams.id);
  ...
  if (isLoading)
    return (
      <div style={{ padding: 40, color: "var(--tx-sub)" }}>
    ...
    <>
      <div className="pagehead">
        <div>
          ...
          <div className="subtitle">
            Partner: {project.company?.name} &middot; ST ID:{" "}
      ...
      <div className="card" style={{ padding: "24px", marginBottom: "24px" }}>
                    ...
                    >
                      ...
                      <td style={{ padding: "12px 0", textAlign: "right" }}>
                        <select
                          value={task.status}
                          onChange={(e) =>
                            handleTaskChange(task.taskId, e.target.value)
                          }
                          style={{
                            padding: "6px 12px",
                            borderRadius: "6px",
        ...
        {(!project.phases || project.phases.length === 0) && (
          <div style={{ color: "var(--tx-sub)" }}>
            No phases generated yet. Project might be older than the sync
            feature.
          </div>
        )}
      </div>
      ...
      <div className="card" style={{ padding: "24px" }}>
        ...
        {project.invoices && project.invoices.length > 0 ? (
            ...
            <tbody>
              {project.invoices.map((inv: any) => (
                ...
                >
                  ...
                  <td style={{ padding: "16px 0", color: "var(--tx-sub)" }}>
          ...
          <div style={{ color: "var(--tx-sub)" }}>
            No invoices synced for this project yet.
          </div>
        )}
      </div>
      ...
      <div className="card" style={{ padding: "24px", marginBottom: "24px" }}>
        ...
        {documents.map((doc: any) => (
          ...
          >
            <span style={{ marginRight: "16px", color: "var(--tx-sub)" }}>
              <Icons.doc />
            </span>
            <div style={{ flex: 1 }}>
              <div
              ...
              <div style={{ color: "var(--tx-sub)", fontSize: "0.85rem" }}>
                {doc.category || "Shared Files"} &middot; {doc.mimeType}{" "}
                &middot; {Math.round(doc.sizeBytes / 1024)} KB
              </div>
            </div>
        ...
        >
          Upload a document to share with the partner.
        </div>
        <label
          className="dropzone"
          id="dropzone"
        ...
        >
          ...
          <span className="dz-text" style={{ textAlign: "center" }}>
            <b style={{ color: "var(--tx-main)", display: "block" }}>
              Upload a document
            </b>
            <span
              className="dz-sub"
              style={{ color: "var(--tx-sub)", fontSize: "0.85rem" }}
            >
              Click to browse &middot; PDF, DOC, etc.
            </span>
          </span>
        </label>
      </div>
      ...
      <div className="card" style={{ padding: "24px" }}>
        ...
        {["Rough-in · Pre-cover Proof", "Finish & Completion", "Other"].map(
          (phase) => {
            const phasePhotos = photos.filter(
              (p: any) => p.phase === phase || (!p.phase && phase === "Other"),
            );
        ...
        >
          Upload a photo to this project.
        </div>
        <label
          className="dropzone"
          style={{
        ...
        >
          ...
          <span className="dz-text" style={{ textAlign: "center" }}>
            ...
            <span
              ...
              style={{ color: "var(--tx-sub)", fontSize: "0.85rem" }}
            >
              Click to browse &middot; JPG, PNG
            </span>
          </span>
        </label>
      </div>
